...
static ngx_int_t ngx_http_mytest_handler(ngx_http_request_t *r)
{
    ...
    /** 必须保证 HTTP 框架在响应改善完毕后关闭已经打开的文件句柄，否则将会出现句柄泄露问题。
     * 设置清理文件最简单方式就是定义一个 ngx_pool_cleanup_t 结构体。将我们刚得到的文件句柄
     * 等信息赋给他，，并将 Nginx 提供的 ngx_pool_cleanup_file 函数设置到它的 handler
     * 回调方法中即可。
     */
