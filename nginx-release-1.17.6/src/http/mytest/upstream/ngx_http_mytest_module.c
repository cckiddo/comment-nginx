...
/*
    上下文在模块解析 ngx_http_mytest() -> ngx_http_mytest_handler(),
    最终调用 ngx_http_set_ctx() 来将构建的 ngx_http_mytest_ctx_t 设置进去。
    同时 handler 还需要设置 upstream 模块中基础的3个回调函数，并初始化 r->upstream
    基础的三个模块 create_request, process_header, finalize_request
*/
typedef struct
{
    /* 因为本例必须要使用上下文才能正确地解析 upstream 上游服务器的响应包，因为 upstream 模块
       每次接收一段 TCP 流时都会回调 mytest 模块的 process_header() 方法解析，这样就
       需要有一个上下文保存解析状态。 */
    ngx_http_status_t           status;
    ngx_str_t					backendServer;
} ngx_http_mytest_ctx_t;

/* 定义一个本模块专属的含 upstream 相关的配置项，在上下文中添加对应的解析函数。
   因为本次示例的配置实际不依赖配置，所以可以设置一些默认值。*/
typedef struct
{
...
static void* ngx_http_mytest_create_loc_conf(ngx_conf_t *cf)
{
    ...
    // 以下简单的硬编码ngx_http_upstream_conf_t结构中的各成员，例如 超时时间都设为1分钟。
    // 这也是http反向代理模块的默认值
...
static char *ngx_http_mytest_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    /* upstream 模块要求 hide_headers 不可以为 NULL, 所以必须要初始化 hide_headers 成员
       通过 ngx_http_upstream_hide_headers_hash() 来初始化，但仅可用在合并配置方法内。 */
...
{
    //发往google（换成baidu）上游服务器的请求很简单，就是模仿正常的搜索请求，
...
/*
    因为 HTTP 的复杂性，无论是响应行还是响应头部都是不定长的，都需要使用状态机来解析。
    所以使用 mytest_process_status_line() 来解析响应行，而使用
    mytest_upstream_process_header() 来解析 http 响应头部。
    实际上，这两个方法是通用的，它们适用于解析所有的 HTTP 响应包，而且这两个方法的代码与
    ngx_http_proxy_module 模块的实现几乎是完全一致的。
*/
static ngx_int_t
mytest_process_status_line(ngx_http_request_t *r)
{
...
mytest_upstream_process_header(ngx_http_request_t *r)
{
    ...
    for ( ;; )
    {
        ...
        //返回 NGX_HTTP_PARSE_HEADER_DONE 表示响应中所有的http头部都解析
...
/* 当请求结束的时候，都会回调 finalize_request 方法，如果我们希望此时释放资源，如打开的句柄
   等，那么可以把这样的代码添加到 finalize_request 方法中 */
static void
...
ngx_http_mytest_handler(ngx_http_request_t *r)
{
    ...
    if (myctx == NULL) {
        myctx = ngx_palloc(r->pool, sizeof( ));
        if (myctx == NULL)
        {
            return NGX_ERROR;
        }
    ...
    // 解析响应行完成之后， 在 mytest_process_status_line()
    // 中设置 mytest_upstream_process_header
