...
struct ngx_http_upstream_s {
	...
	/* 决定发送什么样的请求给上游服务器，在创建 create_request 方法时需要设置它 */
	...
	/* buffer 成员存储接收自上游服务器发来的响应内容，由于它会被复用，
	   所以具有以下多种意义:
	   a)在使用 process_header 方法解析上游响应的包头时，
	   	 buffer 中将会保存完整的响应包头
	   b)当下面的 buffering=1, 而且此时 upstream 是向下游转发上游的包体时，
	   	 buffer 没有意义
	   c)当下面的 buffering=0, buffer 缓冲区会被用于反复地接收上游的包体，
	   	 进而向下游转发
	   b)当 upstream 并不用于转发时， buffer 会被用于反复接收上游和包体，
